...
export async function GET(req: Request) {
  try {
    ...
    const catParamRaw = (searchParams.get("category") || "").trim();
    const catParam = catParamRaw.toLowerCase();
    // If category=FAQ or Questions, treat as knowledge base buckets -> do not filter, return all
    let filtered = faqs;
    if (catParam && catParam !== "faq" && catParam !== "questions") {
      filtered = faqs.filter((f) => f.category.toLowerCase() === catParam);
    }
