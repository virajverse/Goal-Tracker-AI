...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (body && typeof body === "object") {
      const b = body as { context?: unknown; type?: unknown };
      if (typeof b.context === "string") context = b.context;
      if (typeof b.type === "string") type = b.type;
    }

    // Fetch context from Supabase (only if logged in)
    let goals: unknown[] | null = null;
    let recentLogs: unknown[] | null = null;
    if (session?.user_id) {
    ...
    try {
      ...
      for (const g of goals || []) {
        if (g && typeof g === "object") {
          ...
          const tf =
            typeof obj.target_frequency === "string"
              ? `, ${obj.target_frequency}`
              : "";
          contextPrompt += `- ${title}${category}${tf}\n`;
        }
      }
      const logsArr = (recentLogs || []).slice(0, 10);
      if (logsArr.length > 0) {
        contextPrompt += "\nRecent completion history (last 7 days):\n";
        for (const l of logsArr) {
          if (l && typeof l === "object") {
            const obj = l as { log_date?: unknown; is_completed?: unknown };
            const date =
              typeof obj.log_date === "string" ? obj.log_date : "unknown date";
            const done =
              typeof obj.is_completed === "boolean" ? obj.is_completed : false;
            contextPrompt += `- ${date}: ${done ? "✓ completed" : "✗ not completed"}\n`;
          }
        }
      }
