...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabaseAdmin.from('daily_logs').select('*').eq('user_id', session.user_id).order('log_date', { ascending: false });
    if (date) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!goal_id || !log_date || typeof is_completed !== 'boolean') {
      return NextResponse.json({ error: 'goal_id, log_date and is_completed are required' }, { status: 400 });
    }
    ...
    if (existing) {
      const { data, error } = await supabaseAdmin
        ...
        .select('*')
        .single();
      if (error || !data) return NextResponse.json({ error: error?.message || 'Failed to update log' }, { status: 400 });
      ...
      const { data, error } = await supabaseAdmin
        ...
        .select('*')
        .single();
      if (error || !data) return NextResponse.json({ error: error?.message || 'Failed to create log' }, { status: 400 });
      return NextResponse.json({ log: data });
    }
  } catch (err: any) {
    if (err?.message === 'Unauthorized') return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
