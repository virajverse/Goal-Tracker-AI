...
function addDays(d: Date, n: number) { return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() + n)); }
function startOfMonth(d: Date) { return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1)); }
function startOfWeekMonday(d: Date) { const day = d.getUTCDay(); const diff = (day + 6) % 7; return addDays(d, -diff); }
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (type === 'good' || type === 'bad') {
      ...
      if (habitIds.length === 0) return NextResponse.json({ start, end, bucket, series: [], total: 0 });
    }
    ...
    const doneDates = new Set<string>((data || []).filter(l => l.is_done).map(l => l.log_date as string));
    ...
    return NextResponse.json({ start, end, bucket, type: type || 'all', series, total });
  } catch (err: any) {
