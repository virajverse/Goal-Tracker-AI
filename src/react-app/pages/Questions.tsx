...
import { Search, BookOpen, ChevronDown, ChevronUp, Filter, Hash, X } from 'lucide-react';
...
export default function Questions({ forcedCategory }: QuestionsPageProps) {
  ...
  // Keep selectedCategory in sync with forcedCategory (prevents mismatch on route switches)
  useEffect(() => {
  ...
  useEffect(() => {
    filterFAQs();
  }, [faqs, searchTerm, selectedCategory, selectedTags, selectedTopic]);
  ...
  const filterFAQs = () => {
    ...
    if (selectedTopic !== 'all') {
      filtered = filtered.filter((faq) => getFaqTopics(faq).includes(selectedTopic));
    }
  ...
  const topicKeywords: Record<string, string[]> = {
    'Mindset & Motivation': ['mindset','motivation','identity','belief','grit','resilience','focus','confidence'],
    'Health & Fitness': ['health','fitness','workout','exercise','training','cardio','strength','nutrition','diet','steps','sleep'],
    'Productivity & Time Management': ['productivity','time','planning','plan','prioritize','priority','schedule','calendar','deep work','energy management','focus blocks'],
    'Habits & Discipline': ['habit','habits','discipline','streak','2-minute','implementation intention','habit stacking','commitment','routine','cue','friction','temptation bundling'],
    'Emotional Intelligence': ['eq','emotional','emotion','empathy','feedback','conflict','stress','granularity','nvc','labeling','reappraisal','boundaries'],
    'Relationships & Social Life': ['relationship','relationships','social','friend','friends','network','dating','connection'],
    'Career & Skills': ['career','skills','skill','work','job','learning','learn','practice','mentor','portfolio','resume'],
    'Finance & Money': ['finance','money','budget','invest','investing','saving','savings','debt','expense','expenses'],
    'Self-Reflection & Mindfulness': ['mindfulness','meditation','reflect','reflection','journal','journaling','awareness','self-awareness','breathing'],
    'Leadership & Communication': ['leadership','leader','team','management','manage','communication','coach','coaching','nvc','feedback','sbi'],
  };

  const getFaqTopics = (faq: FAQ): string[] => {
    const text = (
      (faq.tags || '') + ' ' + faq.question + ' ' + faq.answer
    ).toLowerCase();
    const topics: string[] = [];
  ...
  return (
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          {/* Search and Filter */}
          <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-6 relative z-[60] isolate overflow-visible">
              ...
              {!forcedCategory ? (
                ...
                <div className="relative">
                  <button
                    ...
                    className="px-4 h-12 w-72 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg flex items-center gap-2 shadow-lg shadow-purple-900/30 hover:from-purple-500 hover:to-indigo-500 focus:outline-none focus:ring-2 focus:ring-purple-400 overflow-hidden"
                    title="Select topic"
                  ...
                  {showCategoryMenu && (
                    <div className="absolute right-0 mt-2 w-72 bg-gradient-to-b from-purple-700 to-indigo-700 text-white rounded-lg shadow-2xl shadow-purple-900/40 z-[9999] max-h-80 overflow-auto ring-1 ring-white/10">
                      <button
                        onClick={() => { setSelectedTopic('all'); setShowCategoryMenu(false); }}
                      ...
                      {topicOptions.map((topic) => (
                        <button
                          key={topic}
                          onClick={() => { setSelectedTopic(topic); setShowCategoryMenu(false); }}
                          className="w-full text-left px-3 py-2 text-sm hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-purple-400/30"
                        >
                          {topic}
                        </button>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {selectedTags.length > 0 && (
                      ...
                      >
                        <span className="inline-flex items-center gap-1"><X className="w-3 h-3" /> Clear</span>
                      </button>
                    )}
                    {allTags.length > 12 && (
                      <button
            ...
            {forcedCategory !== 'FAQ' && searchTerm.trim() !== '' && (
              <div className="mt-3">
                <button
                  onClick={() => askInAIChat(searchTerm)}
                ...
                >
                  <div className="p-4 pt-0">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3 pt-3 border-t border-white/10">
                      {faq.tags && (
                        <div className="flex flex-wrap gap-2">
                          {faq.tags.split(',').map((tag) => (
                            <span
                              key={tag.trim()}
                              className="px-2.5 py-1 text-xs rounded-full bg-purple-900/50 text-purple-200 whitespace-nowrap"
                            >
                              {tag.trim()}
                            </span>
                          ))}
                      ...
                      {forcedCategory !== 'FAQ' && (
                        <button
                          onClick={() => askInAIChat(faq.question)}
                          className="ml-auto px-3 py-1.5 text-sm rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-500 hover:to-indigo-500 border border-white/20"
