    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard
          ...
          value={`${String(todaysCompleted)}/${String(todaysTotal)}`}
          subtitle={`${String(todaysCompletionRate)}% complete`}
          icon={Calendar}
          color="blue"
        />
        ...
        <StatsCard
          ...
          value={`${String(currentStreak)} days`}
          subtitle="Keep it up!"
          icon={TrendingUp}
          color="green"
        />
        ...
        <StatsCard
          ...
          value={
            topPerformingGoals.length > 0
              ? topPerformingGoals[0].title.slice(0, 12) + "..."
              : "N/A"
          }
          subtitle={
            topPerformingGoals.length > 0
              ? `${String(Math.round(topPerformingGoals[0].completion_rate))}% rate`
              : "Complete more goals!"
          }
          icon={Award}
          color="yellow"
          ...
          {progressData.length > 0 ? (
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-8 text-center">
              ...
              <p className="text-white/60">
                Start completing your daily goals to see progress charts!
              </p>
            </div>
          )}
        </div>
                  ...
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-200">
                        {Math.round(goal.completion_rate)}% completion rate
                      </p>
                    </div>
                  </div>
                  <p className="text-xs text-white/60">
                    Completed {goal.completed_count} out of {goal.total_logs}{" "}
                    times
                  </p>
                </div>
              );
