...
import type { Goal, CreateGoal } from "@/shared/types";
import {
  Plus,
  Target,
  TrendingUp,
...
export default function Goals(): React.ReactElement {
  ...
  const filteredGoals = goals.filter((goal) => {
    ...
    const matchesFrequency =
      !filterFrequency || goal.target_frequency === filterFrequency;
  ...
  const avgCompletionRate =
    goalStats.length > 0
      ? Math.round(
          goalStats.reduce((sum, goal) => sum + goal.completion_rate, 0) /
            goalStats.length,
        )
      : 0;
  const topPerformer = goalStats.find(
    (goal) =>
      goal.completion_rate ===
      Math.max(...goalStats.map((g) => g.completion_rate)),
  );

  const handleCreateGoal = async (goalData: CreateGoal): Promise<void> => {
    try {
  ...
  const handleEditGoal = async (goalData: CreateGoal): Promise<void> => {
  ...
  const handleDeleteGoal = async (goalId: number): Promise<void> => {
    if (
      window.confirm(
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatsCard
          ...
          value={`${String(avgCompletionRate)}%`}
        ...
        <StatsCard
          ...
          subtitle={
            topPerformer
              ? `${String(Math.round(topPerformer.completion_rate))}% success`
              : "Complete more goals!"
          }
          icon={Award}
          color="yellow"
            ...
            {goals.length === 0 ? (
              <>
                ...
                <p className="text-white/60 mb-6">
                  Start your journey by creating your first goal. Set clear,
                  achievable targets and track your progress!
                </p>
                <button
                  onClick={() => {
                    setShowForm(true);
      ...
      {showForm && (
        <GoalForm
          onSubmit={(data) => { void handleCreateGoal(data); }}
          onCancel={() => {
            setShowForm(false);
          }}
        />
      ...
      {editingGoal && (
        <GoalForm
          goal={editingGoal}
          onSubmit={(data) => { void handleEditGoal(data); }}
          onCancel={() => {
            setEditingGoal(null);
          }}
        />
