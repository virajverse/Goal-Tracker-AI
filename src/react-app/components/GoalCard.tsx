...
import { Goal, DailyLog } from '@/shared/types';
import { Calendar, Target, Edit3, Trash2, CheckCircle2, Circle } from 'lucide-react';
import { format } from 'date-fns';

interface GoalCardProps {
  goal: Goal;
  dailyLog?: DailyLog;
  onToggleComplete?: (goalId: number, completed: boolean, notes?: string) => void;
...
export default function GoalCard({
  ...
  const frequencyColor = frequencyColors[goal.target_frequency as keyof typeof frequencyColors] || frequencyColors.daily;

  return (
    <div className={`bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-6 hover:bg-white/15 transition-all duration-200 ${compact ? 'p-4' : 'p-6'}`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div className="flex-1">
              <h3 className={`font-semibold text-white ${isCompleted ? 'line-through opacity-75' : ''}`}>
                {goal.title}
              </h3>
              {goal.description && !compact && (
                <p className="text-sm text-purple-100 mt-1">{goal.description}</p>
              )}
            </div>
          </div>

          <div className="flex items-center space-x-2 mb-3">
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${frequencyColor}`}>
